...
def build_dynamic_mock_router(query: str, context: Optional[Dict[str, Any]] = None) -> MagicMock:
    ...
    # 3. Goals
    elif any(k in q_lower for k in ("goal", "emergency fund", "emergency savings", "vacation", "when will i reach", "when will i hit", "how long until", "finish that", "finish my")):
        goal_name = "emergency fund"
        if "vacation" in q_lower:
            goal_name = "vacation"
        elif "savings goal" in q_lower:
    ...
    # 8. UI Control: read_goals
        ...
        or any(k in q_lower for k in (
            ...
            "goals sunao", "show my goals", "show goals", "mera goal"
        ))
    ):
        mock_tool_call = MagicMock()
...
def process_query(
    ...
    # Execute request against /ask endpoint
